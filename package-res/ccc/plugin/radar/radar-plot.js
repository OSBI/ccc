...
/**
 * Initializes a radar plot.
 * 
 * @name pvc.visual.RadarPlot
 * @class Represents a radar plot.
 * @extends pvc.visual.Plot
 */
def('pvc.visual.RadarPlot', pvc.visual.Plot.extend({
    init: function(chart, keyArgs) {

        this.base(chart, keyArgs);

        if(!(chart instanceof pvc.RadarChart))
            throw def.error(def.format("Plot type '{0}' can only be used from within a RadarPlot chart.", [this.type]));
    },
    methods: /** @lends pvc.visual.RadarPlot# */{
        /** @override */
        type: 'radar',
...
pvc.visual.Plot.registerClass(pvc.visual.RadarPlot);
