...
pv.RadarPoint = function() {
    pv.Wedge.call(this);
};

pv.RadarPoint.prototype = pv.extend(pv.Wedge)
    // How much radius to offset the slice from the Radar point center.
    // Must be a value between 0 and the Radar Point's:
...
pv.RadarPoint.prototype.midAngle = function() {
    var instance = this.instance();
    return normAngle(instance.startAngle) + normAngle(instance.angle) / 2;
};

pv.RadarPoint.prototype.defaults = new pv.RadarPoint()
    .extend(pv.Wedge.prototype.defaults)
    .offsetRadius(0);

// -----------

def('pvc.visual.RadarPoint', pvc.visual.Sign.extend({
    init: function(panel, protoMark, keyArgs) {
        var pvMark = protoMark.add(pv.RadarPoint);
    ...
    methods: /** @lends pvc.visual.RadarPoint# */{
