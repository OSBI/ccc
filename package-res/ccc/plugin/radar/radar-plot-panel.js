...
/*
 * Radar chart panel. Generates a radar chart.
 *
...
def
.type('pvc.RadarPanel', pvc.PlotPanel)
.init(function(chart, parent, plot, options) {
    ...
    // Legacy name
    if(!chart.radarChartPanel) chart.radarChartPanel = this;
...
.add({
    ...
    _createCore: function(layoutInfo) {
        ...
        if(this.compatVersion() <= 1) {
            extensionIds.push(''); // let access as "radar_"
                ...
                this.pvLinkLine = new pvc.visual.Line(
                    ...
                    .lockMark('data', function(scene) {
                        ...
                        if(!firstDotScene || !firstDotScene._isFirstDynamicScene) {
                            firstDotScene = new pvc.visual.RadarLinkLineScene(scene, x, y, /* index */ 0);
    ...
    _buildScene: function() {
        var rootScene  = new pvc.visual.RadarRootScene(this);
...
pvc.PlotPanel.registerClass(pvc.RadarPanel);

def
.type('pvc.visual.RadarRootScene', pvc.visual.Scene)
.init(function(panel) {
    ...
    var CategSceneClass = def.type(pvc.visual.RadarCategoryScene)
...
def
.type('pvc.visual.RadarLinkLabelVar') // TODO : Var base class
.add({
...
def
.type('pvc.visual.RadarCategoryScene', pvc.visual.Scene)
.add({
    // extendable
    ...
    layoutI: function(layoutInfo, startAngle) {
        var valueVar = this.vars.value,
            ...
            linkVar = (this.vars.link = new pvc.visual.RadarLinkLabelVar()),
        ...
        new pvc.visual.RadarLinkLineScene(this, elbowX,  elbowY);
        new pvc.visual.RadarLinkLineScene(this, anchorX, elbowY);
    ...
    layoutII: function(layoutInfo) {
        ...
        if(handleWidth > 0)
            new pvc.visual.RadarLinkLineScene(this, targetX - linkVar.dir * handleWidth, targetY);

        new pvc.visual.RadarLinkLineScene(this, targetX, targetY);
...
def
.type('pvc.visual.RadarLinkLineScene', pvc.visual.Scene)
